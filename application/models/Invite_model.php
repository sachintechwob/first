...
class Invite_model extends CI_Model {
  public function __construct()
   ...
   {
      $this->db->select('products.name,image,price,qty,cart.id,product_id');
   ...
   public function cart_items($ids)
   {
      if($ids)
      {
         $detailsIds  = explode(',', $ids);
         foreach ($detailsIds as $id) {
            if($id)
            {            
               $this->db->select('name,image,price,products.id');
